...
export const NestingViewer = ({ nestingResults, selectedVariant }: NestingViewerProps) => {
  ...
  useEffect(() => {
    ...
    const offsetX = (canvas.width - result.sheetWidth * scale) / 2;
    const offsetY = (canvas.height - result.sheetHeight * scale) / 2;
    ...
    for (const placed of result.placedEntities) {
      ...
      // Рисуем родительский контур
      drawEntity(ctx, placed.entity.entity, scale, "#0066cc", 2);

      // Рисуем дочерние контуры
      for (const child of placed.entity.children) {
        drawEntity(ctx, child.entity, scale, "#00aa00", 1);
      }

      ctx.restore();
    }
  }, [nestingResults, selectedVariant, dimensions]);
  ...
  return (
    <div className="flex flex-col h-full gap-3 min-h-0">
      <div ref={containerRef} className="flex-1 flex items-center justify-center min-h-0 overflow-hidden bg-muted/20 rounded-lg">
        <canvas
          ref={canvasRef}
